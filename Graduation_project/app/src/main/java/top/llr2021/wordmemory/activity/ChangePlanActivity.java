...
import top.llr2021.wordmemory.util.JsonHelper;
        ...
        public void handleMessage(@NonNull Message msg) {
            switch (msg.what) {
                case FINISH:
                    ...
                    userConfig1.updateAll("userId = ?", ConfigData.getQQNumLogged() + "");
                    // 删除当天打卡记录
                    ...
                    if (Integer.parseInt(editText.getText().toString().trim()) >= 5
                        ...
                        if (ConfigData.notUpdate == intent.getIntExtra(ConfigData.UPDATE_NAME, 0)) {
                                    ...
                                    thread = new Thread(new Runnable() {
                                        ...
                                        public void run() {
                                            try {
                                                ...
                                                FileUtil.getFileByBytes(response.body().bytes(), getFilesDir()+ "/"
                                                        + ConstantData.DIR_TOTAL, ConstantData.bookFileNameById(currentBookId));
                                                FileUtil.unZipFile(getFilesDir()+ "/" + ConstantData.DIR_TOTAL + "/"
                                                                + ConstantData.bookFileNameById(currentBookId)
                                                        , getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/"
                                                                + ConstantData.DIR_AFTER_FINISH, false);

                                            } catch (Exception e) {
                                            }
                                            JsonHelper.analyseDefaultAndSave(FileUtil.readLocalJson(ConstantData.DIR_TOTAL + "/"
                                                    + ConstantData.DIR_AFTER_FINISH + "/" + ConstantData.bookFileNameById(currentBookId).
                                                    replace(".zip", ".json")));
                                            Message message = new Message();
                                            message.what = FINISH;
                                            handler.sendMessage(message);
                            ...
                            if (userConfigs.get(0).getWordNeedReciteNum() != Integer.parseInt(editText.getText().toString().trim())) {
                                ...
                                userConfig1.updateAll("userId = ?", ConfigData.getQQNumLogged() + "");
                                Toast.makeText(ChangePlanActivity.this, "" + LitePal.where("userId = ?", ConfigData.getQQNumLogged() + "").find(UserConfig.class).get(0).getLastStartTime(), Toast.LENGTH_SHORT).show();
                                Log.d(TAG, "onClick: " + LitePal.where("userId = ?", ConfigData.getQQNumLogged() + "").find(UserConfig.class).get(0).getLastStartTime());
                                // 删除当天打卡记录
                                Calendar calendar = Calendar.getInstance();
                                LitePal.deleteAll(MyDate.class, "year = ? and month = ? and date = ? and userId = ?"
                                        , calendar.get(Calendar.YEAR) + ""
