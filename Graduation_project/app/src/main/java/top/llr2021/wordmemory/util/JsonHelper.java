...
import top.llr2021.wordmemory.object.JsonPhrase;
import top.llr2021.wordmemory.object.JsonSentence;
import top.llr2021.wordmemory.object.JsonTran;
import top.llr2021.wordmemory.object.JsonWord;
...
public class JsonHelper {
    ...
    public static void analyseDefaultAndSave(String jsonContent) {
        ...
        long analystart = SystemClock.currentThreadTimeMillis();
        int flag =0;
        ...
        for (JsonWord jsonWord : jsonWordList) {
            ...
            word.save();
            if(flag%100==0)
                Log.i(TAG,"word耗时"+(SystemClock.currentThreadTimeMillis()-analystart));
            ...
            if(flag%100==0)
                Log.i(TAG,"phrase耗时"+(SystemClock.currentThreadTimeMillis()-analystart));
            // 设置释义
            ...
            jsonTrans.clear();
            if(flag%100==0)
                Log.i(TAG,"全部完成耗时"+(SystemClock.currentThreadTimeMillis()-analystart));
            flag++;
        }
