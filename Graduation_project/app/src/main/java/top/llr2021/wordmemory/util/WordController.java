...
public class WordController {
    ...
    public static void generateDailyReviewWords() {
        ...
        List<Word> littleReviewList = LitePal.where("isLearned = ? and masterDegree < ?", "1", "10").select("wordId","lastMasterTime","masterDegree").find(Word.class);
        // 深度复习候选条件：单词掌握程度达到10并且达到了单词复习的时间
        List<Word> deepReviewList = LitePal.where("masterDegree = ?", "10").select("deepMasterTimes","wordId","lastMasterTime","masterDegree").find(Word.class);
        ...
        for (Word word : deepReviewList) {
            switch (word.getDeepMasterTimes()) {
                case 0:
                    try {
                        // 说明未及时深度复习
                        if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) > 15) {
                            Word newWord = new Word();
                            newWord.setDeepMasterTimes(word.getDeepMasterTimes()-1);
                            newWord.updateAll("wordId = ?", word.getWordId() + "");
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        } else if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) == 15) {
                            // 说明已经到了深度复习的那一天
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        }
                ...
                case 1:
                    try {
                        // 说明未及时深度复习
                        if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) > 60) {
                            Word newWord = new Word();
                            newWord.setDeepMasterTimes(word.getDeepMasterTimes()-1);
                            newWord.updateAll("wordId = ?", word.getWordId() + "");
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        } else if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) == 60) {
                            // 说明已经到了深度复习的那一天
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        }
                ...
                case 2:
                    try {
                        // 说明未及时深度复习
                        if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) > 90) {
                            Word newWord = new Word();
                            newWord.setDeepMasterTimes(word.getDeepMasterTimes()-1);
                            newWord.updateAll("wordId = ?", word.getWordId() + "");
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        } else if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) == 90) {
                            // 说明已经到了深度复习的那一天
                            // 加入复习列表
                            needReviewWords.add(word.getWordId());
                        }
        ...
        for (Word word : littleReviewList) {
            switch (word.getMasterDegree()) {
                case 0:
                    addReviewWord(word,1);
                    break;
                case 2:
                    addReviewWord(word,2-1);
                    break;
                case 4:
                    addReviewWord(word,4-2);
                    break;
                case 6:
                    addReviewWord(word,7-4);
                    break;
                case 8:
                    addReviewWord(word,15-7);
                    break;
            }
        }
        ...
        for(int i:needReviewWords){
            List<Word> words = LitePal.where("wordId = ?", i + "").find(Word.class);
            Word word= words.get(0);
            Log.d(TAG,"需要复习的单词"+word.getWord()+"上次完成时间"+word.getLastMasterTime()+"   当前时间"+TimeController.getCurrentDateStamp());

        }
        Log.d(TAG, needReviewWords.toString());
    }

    public static void addReviewWord(Word word,int IntervalDate){
        try {
            if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) > IntervalDate) {
                ...
                // 加入复习列表
                needReviewWords.add(word.getWordId());
            } else if (TimeController.daysInternal(word.getLastMasterTime(), TimeController.getCurrentDateStamp()) == IntervalDate) {
                Log.d(TAG, "等于加入"+IntervalDate);
                // 说明已经到了深度复习的那一天
                // 加入复习列表
                needReviewWords.add(word.getWordId());
            }
    ...
    public static void reviewOneWordDone(int wordId, boolean isAnswerRight) {
        Log.i(TAG,"一般复习模式被执行");
        List<Word> words = LitePal.where("wordId = ?", wordId + "").select("isLearned","wordId", "word", "masterDegree", "ExamRightNum", "DeepMasterTimes").find(Word.class);
        if (isAnswerRight) {
            // 移除需要复习列表
            ...
            if (words.get(0).getIsLearned() == 0){
                Word newWord = new Word();
                newWord.setIsLearned(1);
                newWord.setLastMasterTime(TimeController.getCurrentDateStamp());
                newWord.setLastReviewTime(TimeController.getCurrentTimeStamp());
                newWord.updateAll("wordId = ?", wordId + "");
                return ;
            }
                // 说明只是浅度复习
