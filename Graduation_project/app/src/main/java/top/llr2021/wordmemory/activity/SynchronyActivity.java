...
import top.llr2021.wordmemory.database.FolderLinkWord;
import top.llr2021.wordmemory.database.Interpretation;
import top.llr2021.wordmemory.database.LearnTime;
import top.llr2021.wordmemory.database.MyDate;
import top.llr2021.wordmemory.database.Phrase;
import top.llr2021.wordmemory.database.Sentence;
...
public class SynchronyActivity extends BaseActivity {
        ...
        public void handleMessage(@NonNull final Message msg) {
            switch (msg.what) {
                ...
                case START_RECOVER_CLOUD:
                    try {
                        ZipUtil.unzip(getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/cloudFiles/" + ConfigData.getQQNumLogged() + ".zip",
                                getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/cloudFiles/" + ConfigData.getQQNumLogged());
                    } catch (IOException e) {
                        Log.i("testss","解压失败");
                        e.printStackTrace();
                    }
                    ...
                    if (currentBookLists != null) {
                                                        ...
                                                        public void onClick(DialogInterface dialog, int which) {
                                                            ...
                                                            new Thread(new Runnable() {
                                                                ...
                                                                public void run() {
                                                                    ...
                                                                    downLoadPath = getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/cloudFiles/" + ConfigData.getQQNumLogged();
                                                                    recoverFromLocal(downLoadPath, currentName);
                                                                    RequestBody formBody = new FormBody.Builder()
                                                                            .add(ServerData.TYPE_NAME, ServerData.RECOVER_TYPE)
                                                                            .add(ServerData.LOGIN_QQ_NUM, ConfigData.getQQNumLogged() + "")
                                                                            .build();
                        ...
                        progressDialog.dismiss();
                        Log.i("testss","currentBookLists为null   "+currentBookLists);
                        Toast.makeText(SynchronyActivity.this, "云端暂无数据，请备份再重试", Toast.LENGTH_SHORT).show();
                    }
                    break;
                case START_SYN_CLOUD:
                    progressDialog.setMessage("准备完毕，正在上传...");
                    uploadInfoFiles(ConfigData.getQQNumLogged() + "");
                    /*RequestBody formBody = new FormBody.Builder()
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<UserConfig> currentConfig = LitePal.where("userId = ?", ConfigData.getQQNumLogged() + "").find(UserConfig.class);
        currentBookName = ConstantData.bookNameById(currentConfig.get(0).getCurrentBookId());

        fileTypeName = ".database";
        filePath = getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/" + ConfigData.getQQNumLogged();
                                ...
                                .enqueue(new Callback() {
                                    ...
                                    public void onResponse(Call call, Response response) throws IOException {
                                        ...
                                        if (responseContent.contains("no_exist") || responseContent.contains("not found")) {
                                            ...
                                            Request request = new Request.Builder()
                                                    .post(formBody)
                                                    .url("http://119.29.184.12:9065/app/recovery")//(ServerData.SERVER_ADDRESS + "/upload/" + ConfigData.getQQNumLogged() + "/" + ConfigData.getQQNumLogged() + ".zip")
                                                    .build();
                                            ...
                                            okHttpClient.newCall(request)
                                                    .enqueue(new Callback() {
                                                        ...
                                                        @Override
                                                        public void onResponse(Call call, Response response) throws IOException {
                                                            FileUtil.getFileByBytes(response.body().bytes(), getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/cloudFiles", ConfigData.getQQNumLogged() + ".zip");
                                                            Message message = new Message();
    ...
    private void saveToLocal(boolean isCloud) {
        ...
        if (isCloud)
            // 压缩文件夹
            try {
                ZipUtil.zip(filePath, zipPath + "/" + ConfigData.getQQNumLogged() + ".zip");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void uploadInfoFiles(String fileName) {
        ...
        try {
            ...
            params.put(ServerData.LOGIN_QQ_NUM, ConfigData.getQQNumLogged());
        } catch (Exception e) {
            Log.d(TAG, "wrong");
