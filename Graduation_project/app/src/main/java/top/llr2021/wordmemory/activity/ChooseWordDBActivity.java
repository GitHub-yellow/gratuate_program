...
import top.llr2021.wordmemory.adapter.WordBookAdapter;
...
import top.llr2021.wordmemory.entity.ItemWordBook;
...
public class ChooseWordDBActivity extends BaseActivity {
    ...
    public void onBackPressed() {
        // 已登录
        if (LitePal.where("userId = ?", ConfigData.getQQNumLogged() + "").find(UserConfig.class).get(0).getCurrentBookId() != -1)
            super.onBackPressed();
        else {
            AlertDialog.Builder builder = new AlertDialog.Builder(ChooseWordDBActivity.this);
            builder.setTitle("提示")
