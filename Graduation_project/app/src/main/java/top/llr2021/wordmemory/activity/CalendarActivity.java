...
import top.llr2021.wordmemory.util.NormalSpan;
...
public class CalendarActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final List<MyDate> myDateList = LitePal.where("userId = ?", ConfigData.getQQNumLogged() + "").find(MyDate.class);
    ...
    private void updateData(CalendarDay date) {
        List<MyDate> myDates = LitePal.where("date = ? and month = ? and year = ? and userId = ?", date.getDay() + "", (date.getMonth() + 1) + "", date.getYear() + "", ConfigData.getQQNumLogged() + "").find(MyDate.class);
        if (myDates.isEmpty()) {
